...
void OitManager::clearResources(VkCommandBuffer cmd, const FrameResources& frame,
  ...
  headClear.image               = frame.oitHeadPointers.image;
  ...
  counterClear.buffer              = frame.oitCounterBuffer.buffer;
  ...
  vkCmdClearColorImage(cmd, frame.oitHeadPointers.image,
                       VK_IMAGE_LAYOUT_GENERAL, &clearColor, 1, &clearRange);
  vkCmdFillBuffer(cmd, frame.oitCounterBuffer.buffer, 0, sizeof(uint32_t), 0u);
...
void OitManager::prepareResolve(VkCommandBuffer cmd,
  ...
  headBarrier.image               = frame.oitHeadPointers.image;
  ...
  nodeBarrier.buffer              = frame.oitNodeBuffer.buffer;
  ...
  counterBarrier.buffer              = frame.oitCounterBuffer.buffer;
