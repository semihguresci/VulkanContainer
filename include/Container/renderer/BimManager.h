...
[[nodiscard]] inline bool sameBimProductIdentity(
    const BimElementMetadata& selected,
...
using BimObjectIndexLookup =
    std::unordered_map<std::string,
                       std::vector<uint32_t>,
                       BimIdentityStringHash,
                       BimIdentityStringEqual>;
...
static_assert(sizeof(BimMeshletGpuCluster) == 48,
              "BimMeshletGpuCluster must match shaders/bim_meshlet_residency.slang.");
static_assert(sizeof(BimMeshletGpuObjectLod) == 32,
              "BimMeshletGpuObjectLod must match shaders/bim_meshlet_residency.slang.");
static_assert(sizeof(BimMeshletResidencyEntry) == 16,
...
static_assert(sizeof(BimMeshletResidencyPushConstants) == 32,
              "BimMeshletResidencyPushConstants must match shaders/bim_meshlet_residency.slang.");
static_assert(sizeof(BimVisibilityGpuObjectMetadata) == 48,
              "BimVisibilityGpuObjectMetadata must match shaders/bim_visibility_filter.slang.");
static_assert(sizeof(BimVisibilityFilterPushConstants) == 64,
              "BimVisibilityFilterPushConstants must match shaders/bim_visibility_filter.slang.");
static_assert(sizeof(BimDrawCompactionPushConstants) == 16,
              "BimDrawCompactionPushConstants must match shaders/bim_draw_compact.slang.");
...
class BimManager {
 public:
  ...
  void updateMeshletResidencySettings(
  ...
  void recordMeshletResidencyUpdate(VkCommandBuffer cmd,
  ...
  [[nodiscard]] const BimElementMetadata* metadataForObject(
      uint32_t objectIndex) const;
  [[nodiscard]] std::span<const uint32_t> objectIndicesForGuid(
      std::string_view guid) const;
  [[nodiscard]] std::span<const uint32_t> objectIndicesForSourceId(
      std::string_view sourceId) const;
  [[nodiscard]] BimElementBounds elementBoundsForObject(
      uint32_t objectIndex) const;
  ...
  [[nodiscard]] const BimMeshletResidencyStats&
  meshletResidencyStats() const {
  ...
  [[nodiscard]] const BimDrawCompactionStats& drawCompactionStats() const {
    return drawCompactionSlots_[static_cast<size_t>(
        BimDrawCompactionSlot::OpaqueSingleSided)].stats;
  }
  [[nodiscard]] const BimDrawCompactionStats& drawCompactionStats(
  ...
  [[nodiscard]] BimDrawBudgetLodStats drawBudgetLodStats(
      const BimDrawFilter& filter) const;
  [[nodiscard]] bool objectMatchesFilter(
      uint32_t objectIndex,
      const BimDrawFilter& filter) const;
  [[nodiscard]] const BimDrawLists& filteredDrawLists(
  ...
  [[nodiscard]] const std::vector<DrawCommand>& floorPlanGroundDrawCommands() const {
  ...
  bool rebuildSectionClipCapGeometry(
      const BimSectionCapBuildOptions& options);
  void clearSectionClipCapGeometry();
  [[nodiscard]] const BimSectionClipCapDrawData& sectionClipCapDrawData()
      const {
 ...
 private:
  ...
  void buildDrawDataFromModel(
      const container::geometry::dotbim::Model& model,
      container::scene::SceneManager& sceneManager);
  void indexElementMetadata(const BimElementMetadata& metadata);
  [[nodiscard]] uint32_t semanticIdForMetadata(
  ...
  void writeMeshletResidencyDescriptorSet(VkBuffer cameraBuffer = VK_NULL_HANDLE,
  ...
  std::vector<std::string> elementStoreys_{};
  std::vector<std::string> elementMaterials_{};
  std::vector<std::string> elementDisciplines_{};
  std::vector<std::string> elementPhases_{};
  std::vector<std::string> elementFireRatings_{};
  std::vector<std::string> elementLoadBearingValues_{};
  std::vector<std::string> elementStatuses_{};
  std::vector<BimStoreyRange> elementStoreyRanges_{};
  ...
  BimDrawFilter cachedDrawFilter_{};
  uint64_t cachedDrawFilterRevision_{std::numeric_limits<uint64_t>::max()};
  BimDrawLists cachedFilteredDrawLists_{};
