...
// The active RenderTechnique builds the concrete RenderGraph. FrameRecorder
// keeps common command recording utilities and executes the prepared graph.
class FrameRecorder {
public:
  FrameRecorder(std::shared_ptr<container::gpu::VulkanDevice> device,
                ...
                GpuCullManager *gpuCullManager, BloomManager *bloomManager,
...
private:
  friend class DeferredRasterTechnique;
  ...
  void drawDiagnosticCube(VkCommandBuffer cmd, VkPipelineLayout layout,
                          uint32_t diagCubeObjectIndex,
                          container::gpu::BindlessPushConstants &pc) const;

  void recordDepthPrepass(VkCommandBuffer cmd, const FrameRecordParams &p,
                          VkDescriptorSet sceneSet) const;

  void recordGBufferPass(VkCommandBuffer cmd, const FrameRecordParams &p,
                         VkDescriptorSet sceneSet) const;
  ...
  void recordShadowPassBody(VkCommandBuffer cmd, const FrameRecordParams &p,
                            uint32_t cascadeIndex) const;
  void
  recordShadowCascadeSecondaryCommandBuffers(const FrameRecordParams &p) const;
  void recordShadowCascadeSecondaryCommandBuffer(VkCommandBuffer cmd,
                                                 const FrameRecordParams &p,
                                                 uint32_t cascadeIndex) const;
  [[nodiscard]] bool canRecordShadowPass(const FrameRecordParams &p,
                                         uint32_t cascadeIndex) const;
  [[nodiscard]] bool
  shouldPrepareShadowCascadeDrawCommands(const FrameRecordParams &p) const;
  [[nodiscard]] bool useGpuShadowCullForCascade(const FrameRecordParams &p,
                                                uint32_t cascadeIndex) const;
  [[nodiscard]] size_t
  shadowCascadeCpuCommandCount(const FrameRecordParams &p,
                               uint32_t cascadeIndex) const;
  [[nodiscard]] bool
  shouldUseShadowSecondaryCommandBuffer(const FrameRecordParams &p,
                                        uint32_t cascadeIndex) const;
  [[nodiscard]] uint64_t
  shadowCascadeDrawCommandSignature(const FrameRecordParams &p) const;
  [[nodiscard]] ShadowCascadeDrawPlannerInputs
  shadowCascadeDrawPlannerInputs(const FrameRecordParams &p) const;

  void prepareShadowCascadeDrawCommands(const FrameRecordParams &p) const;

  void recordLightingPass(
  ...
  void recordBimPointCloudPrimitivePass(VkCommandBuffer cmd,
                                        const FrameRecordParams &p) const;

  void recordBimCurvePrimitivePass(VkCommandBuffer cmd,
                                   const FrameRecordParams &p) const;

  void recordBimSectionClipCapPass(VkCommandBuffer cmd,
                                   const FrameRecordParams &p) const;

  void recordTransformGizmoPass(VkCommandBuffer cmd,
                                const FrameRecordParams &p) const;

  void recordTransformGizmoOverlay(VkCommandBuffer cmd,
                                   const FrameRecordParams &p) const;
  ...
  RenderGraph graph_;
  mutable std::array<std::vector<DrawCommand>,
